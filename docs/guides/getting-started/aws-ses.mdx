...
See ["Create and Send a Message"](./send-message) for details on how to:

- [Build your notification template](./send-message#create-a-notification) and
- [Send a message with the Courier API using cURL](./send-message#send-a-message)

See the [AWS SES integration doc](../providers/email/aws-ses) for profile requirements and override options when sending.
