...
:::info Courier's Segment Integration Help Guide
https://help.courier.com/en/articles/4438651-setting-up-courier-s-segment-integration
:::
...
### Send a notification only if another event doesn't occur

A common use case is to cancel sending a notification if another event occurs. For example, if a user has already completed a task, you may not want to send them a reminder notification. You can use the `cancel` step to cancel an automation that is already in progress to achieve this.

First, configure a new Automation with a delay step. This will delay the automation for a specified amount of time.
...
Then, set up a second automation that will cancel the first automation if the event occurs.
...
You can configure both automations to use triggers as well, for example starting the first automation on a Segment Track event such as `user_signup`, and the second automation on another Segment Track event such as `user_completed_setup`. Ensure that you set up a dynamic cancelationToken for each automation, so that the cancelationToken is unique for each user.