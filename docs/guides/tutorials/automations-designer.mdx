...
An automation template is a re-usable, pre-defined list of automation steps created in the Courier Automation Designer. Published templates are run or "invoked" using Courier's [Automations API](../../../reference/automation/invoke) `automations/{templateId}/invoke` endpoint, or a Segment Event.
