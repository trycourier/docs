...
- `GET /brands/:id` (must have `read:brands` or `read:brand:<id>` for single brand access).
- `PUT | DELETE /brands/:id` (must have `write:brands` or `write:brand:<id>` for single brand access).
