...
Returns the quotient of its operands where the left operand is the dividend and the right operand is the divisor. This helper will throw an errow if it encounters an input value that is not a number. This helper will thrown an error if the divisor is `0`.
