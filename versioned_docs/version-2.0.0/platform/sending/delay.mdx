...
## Delivery Window
You can specify a Delivery Window using a partial implementation of [opening_hours](https://wiki.openstreetmap.org/wiki/Key:opening_hours) (`delay.until`). This also supports timezone offsets from within a user profile `timezone` key or `message.to.timezone`. Based on the current time and the opening_hours string, Courier will calculate if it can send right now (is open) or delay until the next window.
