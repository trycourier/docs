...
This limits the number of messages you can send to an individual account. To work, account must be created using Courier's [Account API](https://www.courier.com/docs/reference/accounts/create-replace/). If you send messages to [users with an account](https://www.courier.com/docs/reference/accounts/user-membership/), the messages will count toward the account limit. Any sends to users without an account will not count toward account limits.
...
<Image
  img={require("../../assets/guides/guardrails/settings-v2.png")}
