...
Courier supports many popular email providers, as well as SMTP. In order for a recipient to receive an email notification, their user profile must include an email address in the `email` property.
