...
Courier support multi-tenant applications with the [Accounts](/reference/accounts) API. A single user can be associated with multiple accounts, and you can specify an `account_id` when sending a message in order to...


```json
"to": {
    "user_id": "luke1234",
    "account_id" : "red-squadron"
}
```
