...
Every Courier tenant has two notification development environments: `Production` and `Test`. The two environments are isolated from one another, and all the assets within them (templates, brands, tags, subscription topics, integrations, API keys, log data, etc.) belong only to that environment.
