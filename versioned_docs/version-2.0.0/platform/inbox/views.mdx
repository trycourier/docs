...
Courier Inbox supports the retrieval and display of different messages, filtered by parameters. Read, Unread, and tagged messages are all examples of

## Notifications (Default)

By default, the Courier inbox provides all messages via its API's sorted by time received. Both read and unread statuses are retrieved, but archived messages are filled out. Pinned messages are returned and displayed at the top and in the APIs will be a seperate object (messages key vs pinned key).
...
The inbox allows you to add then remove tags that weren't part of the `metadata.tags` which is useful for building a custom "Save Message" or " Bookmark"feature. You can add and remove tags then use the same filtering logic as above to filter the inbox by a tag.
