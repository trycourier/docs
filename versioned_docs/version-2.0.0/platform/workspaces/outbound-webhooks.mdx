...
The `data` property in the webhook response payload for the `message:updated` event is identical to the information that is returned from the [GET /message/{message_id}](../../reference/logs/by-id.mdx) endpoint
