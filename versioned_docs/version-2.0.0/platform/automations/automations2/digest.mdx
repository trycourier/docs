...
First step is to configure an automation to add events to the digest. Open an automation template and drag in the `Add to Digest` node under the `Actions` section in the left-hand pane. Select the subscription topic with the digest configured. This will add any event that comes in to that digest **by user**.
