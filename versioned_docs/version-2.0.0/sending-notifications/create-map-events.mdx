...
The Notification ID (or `id` in the Courier API) is a unique identifier for each notification you create in the [Courier Notification Designer](../courier-designer/notification-designer/notification-designer-overview.mdx). You can find this id in the notification settings modal by clicking on the gear on next to the notification name.
...
You can also map a notification template to an existing event in the [notification settings](https://help.courier.com/en/articles/4333977-notification-settings-overview).
<!-- Link follow-up doc -->
<Image
...
The [Events API](/reference/events/intro.mdx) allows you to manage your Events (get and create). When you create an event string to Courier via the API, the call will replace an existing event with the supplied values or create a new event.
