...
A sender can also assign a preferred Integration to a specific recipient, using the [Preferences API](../reference/preferences/intro.mdx). This will override the hierarchy of the Channel Rules and send the message only via the Integration specified in the recipient's [Profile](../reference/profiles/intro.mdx).