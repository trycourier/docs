...
| locales  | object           |          | Region specific content. Elements and Raw fields can be localized. See [locales docs](../locales) for more details.                          |
...
| locales  | object           |          | Region specific content. Elements field can be localized. See [locales docs](../locales) for more details. |
...
Allows raw HTML to be defined and rendered in a notification. This element is only supported
by the email channel. It will not appear in other kinds of notifications (like push or texts).
...
| ------- | ------ | -------- | ------------------------------------------------------------------------- |
| type    | string | required | The type of element. For action elements, this value should be `"text"`.  |
| content | string | required | The html content displayed in the notification.                           |
| locales | object |          | Region specific content. See [locales docs](../locales) for more details. |
...
{
  "type": "html",
  "content": "<h1>This is a notification I sent to {{first_name}}</h1>" // comes from the message.data property (i.e. data.first_name)
}
```
...
{
  "type": "text",
  "content": "<h1>This is a notification I sent to {{first_name}}</h1>",
  "locales": {
    "eu-fr": {
      "content": "<h1>Ceci est une notification que j'ai envoyée à {{first_name}}</h1>"
    }
  }
}
```
