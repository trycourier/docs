...
When the `method` property is defined as `single`, Courier will attempt to the send messages to only one channel. In the example below, it will first attempt SMS. Since  `phone_number` is not included in the user profile, it will attempt to send via email *instead*. If the email is sent successfully, it will not attempt to send a Push notification.
...
When the `method` property is defined as `all`, Courier will attempt to send messages to all listed channels. In the example below, Courier will send the message via email and SMS. Since the Push profile requirements are not completed, it will not be able to send a Push notification.
...
The routing settings created in the designer can always be overridden using the [preferences API](../../reference/preferences/intro.mdx). This will override the hierarchy of the channel rules and send the message only via the Integration specified in the recipient’s [profile](../../reference/profiles/intro.mdx).
