...
The routing settings created in the designer can always be overridden using the [preferences API](../../reference/user-preferences/intro.mdx). This will override the hierarchy of the channel rules and send the message only via the Integration specified in the recipient’s [profile](../../reference/profiles/intro.mdx).
