...
To send a direct message to a Microsoft Teams user, we'll need a Courier recipient with a [Microsoft Teams profile](./#send-a-message-to-a-microsoft-teams-user) that includes a `user_id`, `tenant_id`, `conversation_id`, and `service_url`. We can retrieve these values and save them to a profile using our bot's `add-user` command.
...
You can follow the steps in this [Microsoft Teams article](https://docs.microsoft.com/en-us/microsoftteams/platform/bots/how-to/get-teams-context?tabs=json) in order to fetch the roster or user profile for your bot.
