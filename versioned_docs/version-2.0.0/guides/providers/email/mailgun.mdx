...
It takes Mailgun about a minute to start sending events to the webhooks. So, give it a minute or two, then go back to the Courier Mailgun configuration screen. Activate the toggle switch to "Disable polling for status updates" and press the save button. Now that webhooks are setup, you won't need polling anymore.
