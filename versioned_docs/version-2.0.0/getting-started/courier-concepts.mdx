...
**Keep reading:** [Brands](../platform/content/brands/brands-in-email-notifications.mdx)
...
The [Conditions](../platform/sending/channel-settings.mdx#conditions) tab in the [Notification](../platform/content/notification-settings/general-settings.mdx), [Channel](../platform/sending/channel-settings.mdx) or [Block](../platform/content/content-blocks/content-block-basics.mdx#customizing-block-settings) settings lets you apply conditionals to define when to send a notification, which channel the notification is routed to (even which integration) and which blocks to include.
...
Overrides can be used to change the request body that Courier sends to a provider. Overrides are useful when a field is not yet supported by Courier, or you would like to override the value that Courier generates. Here's an [Override example using SendGrid](/platform/channels/email/sendgrid).
...
**Read more:** [White-labeling](../platform/content/brands/brands-in-email-notifications.mdx)
