---
sidebar_position: 4
slug: /platform/channels/direct-message/microsoft-teams
---
...
Courier provides a [visual template designer](../../platform/content/notification-designer/notification-designer-overview.mdx), so you can send notifications that are formatted professionally. You can add [content blocks](../../platform/content/content-blocks/content-block-basics.mdx) to the template by clicking appropriate icons. If you remove a content block, it is moved to your library in the sidebar and can be dragged back to the template if necessary.

These content blocks can include [variables](../../platform/content/variables/inserting-variables.mdx) using a mustache-like template syntax. Surround text with a single set of curly braces and that text will be interpreted as a variable. For example, you may want to include a `{name}` variable.
...
Courier allows you to preview your notifications using different [Test Events](../../platform/content/notification-designer/how-to-preview-notification.mdx) so you can see how different data will affect the notification. Let's create a Test Event and preview the notification. We'll also use this Test Event when sending the message.
...
To send a direct message to a Microsoft Teams user, we'll need a [Courier Recipient Profile](../../reference/profiles/create.mdx) configured with a [Microsoft Teams object](#send-a-message-to-a-microsoft-teams-user) that includes a `user_id`, `tenant_id`, and `service_url`.
...
To send a message to a Microsoft Teams channel, we'll need a [Courier Recipient Profile](../../reference/profiles/create.mdx) with a [Microsoft Teams object](#send-a-message-to-a-microsoft-teams-channel) that includes a `channel_id`, `tenant_id`, and `service_url`.
