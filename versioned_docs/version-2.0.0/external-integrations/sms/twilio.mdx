...
Debugging is always fun. Here are [some common](https://www.courier.com/error-solutions/twilio-send-message-not-working/) issues that come up when using Twilio.

- Always prefix your `phone_number` values with a `+` to avoid running into `400` error responses.
