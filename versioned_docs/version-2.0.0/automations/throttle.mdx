...
The Throttle step in automation limits the number of events that can pass into the next node within a specific period, dropping any additional events. This prevents customers from receiving too many messages about the same event.
...
In general, a Throttle step can be useful in any situation where you need to control the flow of data or limit the frequency of an action to prevent overload or overwhelm.
...
In this example, the throttle is set to throttle by user_id. You would supply `user_id` in following steps to throttle by user.
